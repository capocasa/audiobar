...
*       // Table name may optionally be provided as fifth parameter
...
class getID3_cached_mysql extends getID3
{
	...
	function analyze($filename) {
		...
		if (file_exists($filename)) {
			...
			// Loopup file
			$result = '';
			$this->cursor = mysql_query("SELECT `value` FROM `$this->table` WHERE (`filename` = '".mysql_real_escape_string($filename)."') AND (`filesize` = '".mysql_real_escape_string($filesize)."') AND (`filetime` = '".mysql_real_escape_string($filetime)."')", $this->connection);
...
echo '.';
	...
	function create_table($drop=false) {
		...
		$this->cursor = mysql_query("CREATE TABLE IF NOT EXISTS `$this->table` (

...
<?php if ($show_play_button): ?><a class="audiobar_play" onclick="top.audiobar.collect_playlist('<?php echo $base ?>'); top.play.location.replace(this.href + (top.audiobar.should_force_flash(<?php echo str_replace('"', "'", json_encode($extensions)) ?>) ? '&forceflash' : '')); return false;" href="<?php echo $wpurl ?>?audiobar=bar&play=<?php echo $base ?>&title=<?php echo urlencode($title) ?>&autoplay=1<?php if (in_array('oga', $extensions)) echo '&altogg' ?>" target="play" class="play">play</a><?php endif; ?><span class="audiobar_title"><?php echo $title ?></span>
